...
const lastIndex = computed(() => {
  const pageSize = table.getState().pagination.pageSize;
  const pageIndex = table.getState().pagination.pageIndex;
  return Math.min((pageIndex + 1) * pageSize, table.getFilteredRowModel().rows.length);
});
</script>

<template>
  <div class="flex flex-col items-center justify-between gap-3 p-5 xs:flex-row">
    <div class="flex-1 hidden text-sm text-muted-foreground xxs:block">
    ...
    <div class="flex items-center space-x-6 lg:space-x-8">
      <div class="flex items-center space-x-2">
        ...
        <Select
          ...
          <SelectContent side="top">
            <SelectItem v-for="pageSize in [5, 10, 20]" :key="pageSize" :value="`${pageSize}`">
              {{ pageSize }}
            </SelectItem>
          </SelectContent>
        </Select>
      ...
      <div class="flex items-center space-x-2">
        <Button
          variant="link"
          class="hidden p-0 w-7 h-7 xxs:flex hover:bg-hover"

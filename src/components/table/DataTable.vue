...
<template>
  ...
  <slot
    ...
    :handleResetFilters="handleResetFilters"></slot>
  ...
  <div
    ...
    <p class="font-medium">{{ table.getSelectedRowModel().rows.length }} selected</p>
    ...
    <Table class="min-w-[1200px]">
      <TableHeader>
        <TableRow v-for="headerGroup in table.getHeaderGroups()" :key="headerGroup.id">
          <TableHead class="py-5" v-for="header in headerGroup.headers" :key="header.id">
            <FlexRender
      ...
      <TableBody>
        ...
        <template v-else>
          <TableRow>
            <TableCell colspan="12" class="p-5">
              <div
                class="py-32 text-center border border-dashed rounded-xl bg-background border-border">
                <p class="mb-4 text-2xl">No data found for '{{ table.getState().globalFilter }}'</p>
